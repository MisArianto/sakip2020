...
class IkuController extends Controller
{
    ...
    {
        return view('app.user.iku.index');
    }
    ...
    public function fecth_ikus($organisasi_no, $tahun)
    {
        $ikus = Iku::leftJoin('indikator_sasaran_renstra as isr', 'isr.id', 'iku.indikator_sasaran_id')
                ...
                ->where('iku.organisasi_no', $organisasi_no)->where('iku.tahun', $tahun)
                ->get();
        ...
        foreach(collect($ikus)->unique('sasaran_id') as $s)
        {
            ...
            foreach(collect($ikus)->unique('indikator_sasaran_id')->where('sasaran_id', $s->sasaran_id) as $is)
            {
                $output .= '<tr style="font-size:  12px;">
                                <td>
                                    <div class="btn-group"><button class="btn btn-info btn-sm" id="handleEdit" url="{{ url("user/iku/edit") }}" data-id="'.$is->id.'" data-indikator_sasaran_id="'.$is->indikator_sasaran_id.'" data-tahun="'.$is->tahun.'" data-alasan="'.$is->alasan.'" data-formulasi="'.$is->formulasi.'" data-sumber_data="'.$is->sumber_data.'" data-keterangan="'.$is->keterangan.'" title="edit '. $is->indikator_sasaran .'"><i class="fa fa-edit cursor"></i></button>
    ...
    {
        $isr = IndikatorSasaran::where('organisasi_no', Auth::user()->organisasi_no)->latest()->get();
    ...
    public function store(Request $request)
    {
            ...
            if($cek)
            {
                ...
                return $this->fecth_ikus(Auth::user()->organisasi_no, $request->tahun_emit);
            }
    ...
    public function destroy(Request $request)
    {
        ...
        return $this->fecth_ikus(Auth::user()->organisasi_no, $request->tahun);
    }
}
