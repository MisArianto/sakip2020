...
class PKEselon2Controller extends Controller
{
    ...
    {
        return view('app.user.PKEselon2.index');
    }

    public function fetch_indikator_sasaran()
    {
        $indikator_sasarans = IndikatorSasaran::where('organisasi_no', Auth::user()->organisasi_no)->latest()->get();
    ...
    public function fetch_eselon2($organisasi_no, $tahun)
    {
        ...
        if($tahun == '2021')
        {
            $eselon2 = PK2::leftJoin('indikator_sasaran_renstra as is','is.id','pk_eselon_2.indikator_sasaran_id')
                        ->leftJoin('program90 as p', 'p.kode_program2', 'pk_eselon_2.program_no')
                        ->leftJoin('sasaran_renstra as sr','sr.id','is.sasaran_id')
                        ->leftJoin('jabatan_simpeg as j','j.id','pk_eselon_2.jabatan_id')
        ...
        foreach(collect($eselon2)->unique('sasaran_id') as $data1){
            $output .= '<tr>
                ...
                $output .= '<tr>
                                <td>
                                    <div class="btn-group"><button class="btn btn-info btn-sm" id="handleEdit" url="{{ url("user/perjanjian-kinerja/eselon-2/edit") }}" data-id="'.$p->id.'" data-indikator_sasaran_id="'.$p->indikator_sasaran_id.'" data-tahun="'.$p->tahun.'" data-program_no="'.$p->program_no.'" data-jabatan_id="'.$p->jabatan_id.'" data-satuan_id="'.$p->satuan_id.'" data-target="'.$p->target.'" data-pagu="'.$p->pagu.'" title="edit '. $p->indikator_sasaran .'"><i class="fa fa-edit cursor"></i></button>
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            if($cek)
            {
                ...
                return $this->fetch_eselon2(Auth::user()->organisasi_no, $request->tahun_emit);
            }
    ...
    public function destroy(Request $request, $id)
    {
        try {
            ...
            return $this->fetch_eselon2(Auth::user()->organisasi_no, $request->tahun_filter);
