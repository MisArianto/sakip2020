...
<br>
    <div class="tab-pane active" id="horizontal-form">
        <form action="{{ url('perencanaan/renstra-program-indikator/store') }}" class="form-horizontal" method="POST">
            ...
            <div class="form-group">
                <label for="program_no_user" class="col-sm-2 control-label">{{ __('Program') }}</label>
                <div class="col-sm-8">
                    <select id="program_no_user" class="form-control {{ $errors->has('program_no_user') ? ' is-invalid' : '' }} select2" name="program_no_user">
                    <option value="">--Pilih Program--</option>
                    @foreach($programs as $program)
                    <option {{ old('program_no_user') ? 'selected' : '' }} value="{{ $program->program_no }}">{{ $program->program_nama }}</option>
                    @endforeach
                    </select>
                    @if ($errors->has('program_no_user'))
                        <span class="text-danger">
                            <strong>{{ $errors->first('program_no_user') }}</strong>
                        </span>
                    @endif
                </div>
            </div>
            ...
            <div class="form-group">
                <label for="indikator_program_nama" class="col-sm-2 control-label">{{ __('Indikator Program') }}</label>
                <div class="col-sm-8">
...
$('#organisasi_no').on('change', function(){
    ...
    $.ajax({
        url: "{{ url('perencanaan/renstra-program-indikator/ajaxProgramIndikator') }}"+'/'+org_no,
        dataType: 'json',
