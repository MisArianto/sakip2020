...
<div class="box box-default" style="padding: 15px;">
	<h3> Program dan Kegiatan Renstra  &nbsp;
      @if(Auth::user()->level != 3)
        <a href="{{url ('perencanaan/renstra-program/create')}}" data-toggle="tooltip" title="Tambah Program dan Kegiatan"><button class="btn btn-sm btn-primary"><i class="fa fa-plus">&nbsp;</i>Kegiatan</button></a>
        <a href="{{url ('perencanaan/renstra-program-indikator/tambah')}}" data-toggle="tooltip" title="Tambah Indikator Program"><button class="btn btn-sm btn-primary"><i class="fa fa-plus">&nbsp;</i> Indikator Program</button></a>
      @endif
    </h3>

@if(Auth::user()->level == 1)
<form action="{{ url ('perencanaan/renstra-program/dataProgram')}}" method="POST">
	{{ csrf_field() }}
	<div class="row">
          <div class="col-md-12">
		...
		<table class="table table-responsive table-hover table-bordered" id="progKegRenstra" style="font-size: 11px;">
			...
			<tbody>
				...
				{{-- @foreach($indikator_sasaran as $is)
					...
					<tr style="font-size: 12px;font-weight: bold; background-color: #75b4c3;">
						{{-- <td></td> --}}
						{{-- <td>{{$p->program_no}}</td> --}}
