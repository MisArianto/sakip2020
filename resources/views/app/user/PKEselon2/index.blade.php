...
<div class="layar" id="loading-off">
    <div class="card card shadow mb-4">
        ...
        <div class="card-body">
            <form id="form-filter" url="{{ url('user/perjanjian-kinerja/eselon-2/fetch')}}" class="mb-5">
                @csrf
              ...
              <!-- include table -->
              @include('app.user.PKEselon2._table')
            </div>
...
<!-- inlcude modal -->
@include('app.user.PKEselon2._modal')
...
<script>
    $(document).ready(function(){
        ...
        $(document).on('change', '#form #tahun', function(){
            let tahun = $(this).val()
        ...
        $(document).on('click', '#handleDeletePk', function(){
	        ...
	        Swal.fire({
	            ...
	            if (result.value) {
	              $.ajax({
	                    type: 'POST',
	                    url: '{{ url("user/perjanjian-kinerja/eselon-2/destroy") }}'+'/'+id,
	                    data: form,
	                    headers: token,
	                    success: function(res){
        ...
        $(document).on('change','#form #program_no', function(){

            let program_no = $('#form #program_no option:selected').val();
            ...
            $.ajax({
                type: 'POST',
                url: `{{ url('user/perjanjian-kinerja/eselon-2/dpa') }}`,
                headers: token, 
                data: obj,
                success: function(res){
    ...
    function longValueEdit(value){
        ...
        fetch_program(tahun, program_no)
    ...
    function fetch_indikator_sasaran(){
        ...
        $.ajax({
            url: `{{ url('user/perjanjian-kinerja/eselon-2/fetch-indikator_sasaran') }}`,
            dataType: 'json',
            success: function(res)
            {
                let output = '';
    ...
    function fetch_program(tahun, program_no = 0){
        $('#myModal #modal-loading-on').removeClass('layar')
        $('#myModal #modal-loading-off').addClass('layar')

        $.ajax({
            url: `{{ url('user/perjanjian-kinerja/eselon-2/fetch-program') }}/${tahun}`,
            dataType: 'json',
            success: function(res)
            {
                let output = '';
                    output += '<option value="">Pilih</option>';
                     $.each(res.programs, function(i, item){
                            output += '<option '
                            if(program_no == item.program_no)
                            {
                                output += 'selected '
                            }
                            output += 'value="'+item.program_no+'">'+item.program_nama+'</option>';
                     });
    ...
    function fetch_jabatan(){
        ...
        $.ajax({
            url: `{{ url('user/perjanjian-kinerja/eselon-2/fetch-jabatan') }}`,
            dataType: 'json',
            success: function(res)
            {
                let output = '';
    ...
    function fetch_satuan(){
        ...
        $.ajax({
            url: `{{ url('user/perjanjian-kinerja/eselon-2/fetch-satuan') }}`,
            dataType: 'json',
            success: function(res)
            {
                let output = '';
