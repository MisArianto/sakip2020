...
<div class="layar" id="loading-off">
    <div class="card card shadow mb-4">
        ...
        <div class="card-body">
            <form id="form-filter" url="{{ url('user/perjanjian-kinerja/eselon-3/fetch')}}" class="mb-5">
                @csrf
              ...
              <!-- include table -->
              @include('app.user.PKEselon3._table')
            </div>
...
<!-- inlcude modal -->
@include('app.user.PKEselon3._modal')
...
<script>
    $(document).ready(function(){
        ...
        $(document).on('click', '#handleDeletePk', function(){
	        ...
	        Swal.fire({
	            ...
	            if (result.value) {
	              $.ajax({
	                    type: 'POST',
	                    url: '{{ url("user/perjanjian-kinerja/eselon-3/destroy") }}'+'/'+id,
	                    data: form,
	                    headers: token,
	                    success: function(res){
        ...
        $(document).on('change','#form #program_no', function(){
            ...
            $.ajax({
                type: 'POST',
                url: `{{ url('user/perjanjian-kinerja/eselon-3/dpa') }}`,
                headers: token, 
                data: obj,
                success: function(res){
    ...
    function fetch_program(tahun, program_no = 0){
        ...
        $.ajax({
            url: `{{ url('user/perjanjian-kinerja/eselon-3/fetch-program') }}/${tahun}`,
            dataType: 'json',
            success: function(res)
            {
                let output = '';
    ...
    function fetch_jabatan(){
        ...
        $.ajax({
            url: `{{ url('user/perjanjian-kinerja/eselon-3/fetch-jabatan') }}`,
            dataType: 'json',
            success: function(res)
            {
                let output = '';
    ...
    function fetch_satuan(){
        ...
        $.ajax({
            url: `{{ url('user/perjanjian-kinerja/eselon-3/fetch-satuan') }}`,
            dataType: 'json',
            success: function(res)
            {
                let output = '';
