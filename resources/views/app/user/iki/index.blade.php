...
<div class="layar" id="loading-off">
    <div class="card card shadow mb-4">
        ...
        <div class="card-body">
            <form id="form-filter" url="{{ url('user/iki/get-pegawai')}}" class="mb-5">
                @csrf
...
<!-- inlcude modal -->
@include('app.user.iki._modal')


<!-- include modal detail -->
@include('app.user.iki._modalDetail')
...
<script>
                ...
                success: function(res){
                  ...
                  let output = ''
                     ...
                     $.each(pegawais, function(i, item){
                        ...
                        output += '<div class="btn-group"><button class="btn btn-warning btn-sm" id="handleDetailIki" url="{{ url('user/iki/detail') }}" data-id="'+item.id+'" title="detail '+ item.nama +'"><i class="fa fa-eye cursor"></i></button>';
                        output += '<button class="btn btn-info btn-sm" id="handleEditIki" url="{{ url('user/iki/filter-iki') }}" data-id="'+item.id+'" data-nama="'+item.nama+'" data-jabatan="'+item.jabatan+'" title="edit '+ item.nama +'"><i class="fa fa-edit cursor"></i></button>';
                        output += '<button class="btn btn-danger btn-sm" id="handleDelete" data-id="'+item.id+'" url="{{ url('user/iki/destroy') }}" title="delete '+ item.nama +'"><i class="fa fa-trash cursor"></i></button></div>';
                        output += '</td>';
                ...
                success: function(res)
                {
                    ...
                    table += `
                        ...
                        <td>
                          ...
                          <button type='button' id="handleDeleteIki" data-id="${item.id}" data-index="${i}" url="{{ url('user/iki/destroy') }}" class='btn btn-danger' data-row='row${item.id}'>
                ...
                success: function(res)
                {
                    ...
                    let output = ''
                     ...
                     $.each(res.pegawais, function(i, item){
                        ...
                        output += '<div class="btn-group"><button class="btn btn-warning btn-sm" id="handleDetailIki" url="{{ url('user/iki/detail') }}" data-id="'+item.id+'" title="detail '+ item.nama +'"><i class="fa fa-eye cursor"></i></button>';
                        output += '<button class="btn btn-info btn-sm" id="handleEditIki" url="{{ url('user/iki/filter-iki') }}" data-id="'+item.id+'" data-nama="'+item.nama+'" data-jabatan="'+item.jabatan+'" title="edit '+ item.nama +'"><i class="fa fa-edit cursor"></i></button>';
                        output += '<button class="btn btn-danger btn-sm" id="handleDeleteUtama" data-id="'+item.pegawai_id+'" url="{{ url('user/iki/destroy') }}" title="delete '+ item.nama +'"><i class="fa fa-trash cursor"></i></button></div>';
                        output += '</td>';
                        output += '<td>'+item.nama+'</td>';
                        output += '<td>'+item.jabatan+'</td>';
    ...
    function fetch_pegawais_pimpinans(){
        $('#myModal #modal-loading-on').removeClass('layar')
        $('#myModal #modal-loading-off').addClass('layar')

        $.ajax({
            url: `{{ url('user/iki/fetch-pegawai-pimpinan') }}`,
            dataType: 'json',
            success: function(res)
            {
                let output = '';
